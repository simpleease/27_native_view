...
- (void)onMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if ([[call method] isEqualToString:@"changeBackgroundColor"]) {
        ...
        UIColor *clr = [UIColor colorWithRed:[dict[@"r"] floatValue] green:[dict[@"g"] floatValue] blue:[dict[@"b"] floatValue] alpha:[dict[@"a"] floatValue]];
        CGFloat sz = [dict[@"sz"] floatValue];
        _templcateView.backgroundColor = clr;//[UIColor blueColor];
        result(@0);
    } else {
